...
- In the file `/lib/loaders.py`, we need to change `buildings_missing` to `buildings_removed` to match the directory name of the dataset.
...
- `png/buildings_difference[number]`: building map with **[number]** problematic points, which can be used to simulate real-world measurement errors. (This was not used in the original repository training.) 
- `png/buildings_removed[number]`: building map with **[number]** removed points, which can be used to simulate real-world measurement errors. (The original repository used `missing`)
- `png/cars`: car map
